...
echo "SSL_VERSION = UNDETERMINED" >> $SQLNET_ORA
echo "SSL_CLIENT_AUTHENTICATION = FALSE" >>  $SQLNET_ORA
echo "TCP.VALIDNODE_CHECKING = NO" >>  $SQLNET_ORA
...
\
" >>  $SQLNET_ORA
echo "SQLNET.AUTHENTICATION_SERVICES = (TCPS,BEQ,NTS)" >>  $SQLNET_ORA

# listener.ora
LSNR_ORA=$TNS_ADMIN/listener.ora
echo "SSL_CLIENT_AUTHENTICATION = FALSE" >> $LSNR_ORA
echo "WALLET_LOCATION = \
  (SOURCE = \
    (METHOD = FILE) \
    (METHOD_DATA = \
...
echo "SSL_VERSION = UNDETERMINED"  >> $LSNR_ORA
